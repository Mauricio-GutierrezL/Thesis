...
This repository is a thesis-specific derivative of the original
[`qml-benchmarks`](https://github.com/XanaduAI/qml-benchmarks) repository by
Xanadu and the authors of
"Better than classical? The subtle art of benchmarking quantum machine learning models".
It is not a GitHub fork and does not preserve the original commit history.
The relation to the upstream project should therefore be understood through this
explicit reference rather than through GitHub's fork metadata.

Most of the generic benchmarking infrastructure, dataset interfaces, model
interfaces, and baseline benchmark scripts are inherited from the upstream
repository. This repository should be read as a modified snapshot used for the
thesis work, not as a wholly independent codebase.
...
## Thesis-specific changes

Most contents of this repository are identical or only lightly adapted relative
to the upstream `qml-benchmarks` project. The main thesis-specific additions and
modifications are:

- `src/qml_benchmarks/models/circuit_centric.py`:
  modified to implement the controlled CCC ansatz and the half-separable and
  fully separable CCC variants studied in the thesis.
- `src/qml_benchmarks/models/iqp_kernel.py`:
  modified to implement the separable and half-separable IQP-kernel variants,
  including the seeded random subset construction for reduced ZZ entanglement.
- `thesis/`:
  thesis manuscript sources, generated datasets, run scripts, logs, custom plot
  scripts, and thesis-specific result aggregation.
- `thesis/plot/*.py`:
  thesis-specific plotting scripts used to generate benchmark-example panels,
  ranking plots, score plots, forest plots, and related summary figures.
- `thesis/generate_paper_benchmarks.py` and `thesis/runs/`:
  thesis-specific dataset generation and experiment orchestration utilities.

Unless a file or folder is listed above, it should be assumed to come primarily
from the upstream repository, possibly with local adjustments for running the
thesis experiments.

## Figure and plot reproducibility

Figures used in the thesis come from two different sources.

- Script-generated thesis plots:
  many benchmark result plots and benchmark-example figures can be generated
  from scripts under `thesis/plot/`, such as `plot_scores.py`,
  `plot_rankings_total.py`, `plot_ccc_forest.py`, `plot_gap_vs_test_acc.py`,
  and `plot_benchmark_examples.py`.
- Static or manually prepared figure assets:
  some figures included in the LaTeX thesis are stored directly under
  `thesis/Latex/figures/` and are not currently regenerated by a script in this
  repository. This includes, for example, `grover_circuit.png`,
  `strongly_entangling_layers.png`, `iqp.png`, and the controlled-CCC diagram
  assets under `thesis/Latex/figures/controlled_ccc/`.

As a result, not every figure appearing in the thesis PDF can be reproduced by a
single script from this repository alone. Where a figure is static, it should be
understood as an included asset rather than an output of an in-repo generation
pipeline.
