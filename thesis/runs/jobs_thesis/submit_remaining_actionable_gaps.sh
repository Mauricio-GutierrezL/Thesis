...
if (( ccc_hps_tasks > 0 )); then
  ...
  ccc_hps_jobid="$(
    sbatch --parsable --array="0-$(( ccc_hps_tasks * ccc_combo_count - 1 ))%2" \
      "$ROOT/thesis/runs/jobs_thesis/hps_cccs_bars_and_stripes_remaining_chunked.sbatch"
  )"
...
if (( ccc_future_score_tasks > 0 )); then
  ...
  if [[ -n "$ccc_hps_jobid" ]]; then
    sbatch --dependency="afterok:$ccc_hps_jobid" \
      ...
      "$ROOT/thesis/runs/jobs_thesis/score_cccs_bars_and_stripes_remaining_chunked.sbatch"
  else
