...
This chapter describes the experimental setup used to study the role of entanglement in the Circuit-Centric Classifier (CCC). The main goal is not only to compare models by final accuracy, but to construct controlled ablations of the same quantum architecture and examine how performance changes as entanglement is reduced. To keep the comparison as fair as possible, all variants are evaluated within the same benchmarking framework, on the same datasets, and under the same training and hyperparameter-search procedure.

\section{Experimental Setup}

All experiments are carried out within the benchmarking framework of \textcite{bowles2024better}, which provides a common interface for datasets, model evaluation, hyperparameter search, etc. In this thesis, I keep the overall benchmark structure of the paper and modify only the model family under study. Concretely, the focus is the Circuit-Centric Classifier introduced by \textcite{schuld2018circuit} and its dequantized variants implemented in the same codebase.
...
The \textbf{bars and stripes} benchmark moves from geometric toy data to simple image structure. Samples are noisy images belonging either to the class of vertical bars or to the class of horizontal stripes. This task is especially relevant for convolutional or image-structured models, since success depends on recognizing simple translation-invariant patterns rather than on modeling complicated global geometry.

The \textbf{downscaled MNIST} benchmark provides a more realistic classification task while remaining tractable for QML simulations. It uses the handwritten-digit problem restricted to digits 3 versus 5, but in reduced form. In the reference benchmark, this appears both as low-dimensional PCA representations and as coarse-grained image representations for convolutional models. This dataset family is useful because it bridges the gap between highly controlled synthetic tasks and simplified real-image data.

The \textbf{hidden manifold} benchmarks are designed to mimic settings in which the observed data live in a high-dimensional space even though the true structure is governed by fewer latent degrees of freedom, as is often the case in image and signal data. Two variants are particularly relevant. In \textbf{hidden manifold model}, the manifold dimension is held fixed while the visible input dimension increases. The underlying problem therefore remains the same, but it is embedded into a larger ambient space. In \textbf{hidden manifold diff}, by contrast, the visible dimension is fixed while the manifold dimension changes. Here the observed number of features stays constant, but the latent structure itself becomes more complex, making the task intrinsically harder.

The \textbf{two curves} benchmark targets nonlinear geometric classification. The two classes lie on distinct curved manifolds, so the relevant structure is no longer a straight separating boundary but curvature and relative geometric position. In intuitive terms, the task asks whether a model can decide to which bent path a point belongs.

Finally, the \textbf{hyperplanes and parity} benchmark introduces a compositional decision rule. Labels are defined by the parity of several hyperplane tests: a point is classified according to whether it lies on the positive side of an even or odd number of hyperplanes. The model therefore cannot solve the task by learning a single separating surface, but must instead combine several local yes-or-no boundary relations into one global label rule. In the variant \textbf{hyperplanes diff}, the difficulty is increased by raising the number of hyperplanes.
...
\section{Quantum Model: Circuit-Centric Classifier}

The main quantum model studied in this thesis is the Circuit-Centric Classifier (CCC), following the architecture proposed by \textcite{schuld2018circuit}. It follows the general ideas of data embedding, variational ans\"atze, and measurement-based predictions, which were introduced in the previous chapter.

\subsection{Input Encoding}

CCC uses amplitude encoding together with repeated copies of the encoded input state. In practice, if the classical feature dimension is not already a power of two, the input is padded accordingly, and the resulting state is embedded multiple times through a tensor-product construction. If $d$ denotes the number of input copies and $N$ the number of classical features, then the total number of qubits in the model is
\[
M = d \cdot \left\lceil \log_2(N) \right\rceil,
\]
which grows both with the feature dimension and with the copy hyperparameter. This part of the model is inherited directly from the original CCC and is identical for the original and dequantized variants, so that the comparison isolates the effect of the trainable circuit rather than the feature map.

\subsection{Variational Circuit and Prediction Rule}

In the reference implementation used here, the original CCC employs PennyLane's \texttt{StronglyEntanglingLayers} as its variational block. The model output is obtained from the expectation value of Pauli-$Z$ on the first qubit, together with a trainable scalar bias. The resulting score has the form
\[
f_\theta(x) = \langle Z_0 \rangle + b,
\]
and its sign determines the predicted class. For the purposes of this thesis, the important point is that the prediction rule is kept fixed across all CCC variants. The only systematic modification concerns the internal entangling structure of the variational circuit.

\section{Dequantized CCC Variants}

The methodological contribution of this thesis is not a new model family, but a controlled modification of CCC designed to reduce or remove entanglement while preserving the rest of the architecture as far as possible. This follows the ablation perspective advocated by \textcite{bowles2024better}.
...
\]
The active edges are chosen by seeded pseudorandom sampling, making the construction reproducible across runs while avoiding the same deterministic subset in every layer.
...
\begin{itemize}
  ...
  \item larger values of $\alpha$ increase the density of active entangling gates.
\end{itemize}
...
The first is the \emph{separable} CCC. Here the entanglement-control parameter is set to $\alpha = 0$. As a result, no two-qubit gates are applied anywhere in the variational block. The circuit therefore remains fully local, and this constitutes the strongest dequantization step considered in the thesis.
...
The CCC implementations are trained as binary classifiers with labels in $\{-1,+1\}$. The model parameters consist of the variational circuit parameters together with the scalar bias term. Hyperparameters such as the number of input copies, the number of variational layers, optimization settings, and related training parameters are selected through the same search procedure used by the benchmark framework. This point matters because, as emphasized by \textcite{bowles2024better}, hyperparameter tuning can strongly affect benchmark outcomes in QML.
