...
\title{Quantifying the Effect of Quantum Operations in Machine Learning by Comparison to Classically Simulable Variants}
...
% Chapters
\chapter{Introduction and Related Literature}
...
We briefly review the basic linear algebra concepts used throughout this thesis, following the exposition in \textcite[Chapter~2]{nielsen_chuang_qcqi}.
...
A map between vector spaces is linear if it respects these operations,
\[
L(\alpha v + \beta w) = \alpha L(v) + \beta L(w).
\]
The standard Hermitian inner product is
\[
\langle v,w\rangle = \sum_i \overline{v_i} w_i,
\]
which induces the norm
\[
\|v\| = \sqrt{\langle v,v\rangle}.
\]
Two vectors are orthogonal if $\langle v,w\rangle = 0$, and a vector is normalized if $\|v\| = 1$. We often write vectors and inner products in bra--ket notation, where a column vector is denoted $\lvert v\rangle$, its conjugate transpose is $\langle v\rvert$, and the inner product is $\langle v \vert w\rangle$.
...
\end{itemize}
Equivalently, this measurement can be described by the family of orthogonal projectors
\[
P_i = \lvert i \rangle \langle i \rvert, \qquad p(i) = \langle \psi \vert P_i \vert \psi \rangle
\]
and normalized post-measurement states
\[
\lvert \psi_i \rangle = \frac{P_i \lvert \psi \rangle}{\sqrt{p(i)}}.
\]
More general measurements can be modeled by a collection of measurement operators $\{M_m\}$ satisfying $\sum_m M_m^\dagger M_m = I$, but in this thesis we will primarily use projective measurements in the computational basis.
...
\subsection{How is data encoded?}
...
\subsubsection{VQCs: Variational Quantum Circuits}
...
\input{figures/benchmark_examples_snippet}
...
The first quantum model family studied in this thesis is the Circuit-Centric Classifier (CCC), following the architecture proposed by \textcite{schuld2018circuit}. Conceptually, CCC belongs to the variational, or quantum-neural-network, branch of QML discussed in Section~\ref{sec:qml}. As such, it consists of trainable parameters arranged in layers and optimized from data to produce a classification score. However, instead of applying affine transformations and classical activation functions, CCC uses a parametrized quantum circuit acting on an encoded quantum state. In this sense, it follows the general framework of data embedding, variational ans\"atze, and measurement-based prediction introduced in the previous chapter.
...
\begin{figure}[H]
    ...
    \begin{tabular}{cc}
        ...
        \multicolumn{2}{c}{\includegraphics[width=0.53\textwidth,keepaspectratio]{figures/controlled_ccc/controlled_ccc_alpha_0p0_r3.png}} \\[0.2em]
        ...
        \multicolumn{2}{c}{\includegraphics[width=0.53\textwidth,keepaspectratio]{figures/controlled_ccc/controlled_ccc_alpha_0p5_r3.png}} \\[0.2em]
        ...
        \multicolumn{2}{c}{\includegraphics[width=0.53\textwidth,keepaspectratio]{figures/controlled_ccc/controlled_ccc_alpha_1p0_r3.png}}
    \end{tabular}%
    }
...
\subsection{CCC Variants}

Two dequantized variants are considered in this work, namely, \emph{CircuitCentricClassifierHalfSeparable} and \emph{CircuitCentricClassifierSeparable}.

In the separable version, the entanglement-control parameter is set to $\alpha = 0$. As a result, no two-qubit gates are applied anywhere in the variational block. The circuit therefore remains fully local, and this constitutes the strongest dequantization step considered.

The \emph{half-separable} has the entanglement-control parameter set to $\alpha = 0.5$, so that approximately half of the candidate two-qubit couplings in each layer remain active. This model occupies an intermediate position between the original entangling CCC and the fully separable version. It is methodologically useful because it allows us to test whether performance degrades gradually as entanglement is reduced, or whether most of the observed behavior can already be reproduced with only limited entangling structure. It uses a seeded random 50\% subset instead, which preserves reproducibility.
...
Equivalence to the original CCC is recovered when the controlled ansatz is used at full entanglement, that is, when every candidate entangling edge is kept active in each layer. In that case, the modified implementation applies the same single-qubit rotation block on every qubit, uses the same layer-wise interaction range \(r\), and places the same two-qubit gates on the same wire pairs. The controlled ansatz therefore reduces gate-by-gate to the original entangling CCC.
...
The second quantum model family is the IQP kernel classifier from \textcite{havlicek2019supervised}, implemented here through the \texttt{IQPKernelClassifier} already present in the benchmark codebase. This family is included so that the entanglement-ablation question can be asked not only for a variational classifier, but also for a quantum-kernel model. In contrast to CCC, the IQP kernel model does not learn circuit parameters directly. Instead, it replaces the classical similarity function used in kernel methods, discussed in Section~\ref{subsec:kernel-methods}, by one induced through a quantum feature map.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.70\linewidth]{figures/iqp.png}
    \caption{Example of an IQP embedding circuit with three input features and two repeated embedding blocks. Adapted from PennyLane~\parencite{pennylane_iqp_embedding}.} 
    \label{fig:iqp-embedding}
\end{figure}

The feature map follows the logic of PennyLane's \texttt{qml.IQPEmbedding} template \parencite{pennylane_iqp_embedding}. In each repeat, the template applies a layer of Hadamard gates and single-qubit \(R_Z\) rotations that encode the individual features, followed by ZZ interactions that encode pairwise feature products. The resulting circuit defines a kernel of the form
\[
    k(x,x') = |\langle 0|U(x')^{\dagger}U(x)|0\rangle|^2,
\]
which is then passed to a classical support-vector machine with a precomputed kernel matrix. In other words, the quantum circuit defines the feature space, while the final classifier remains a classical SVM.

\section{Entanglement Ablation}
For the IQP family, the ablation modifies the entangling structure inside this embedding while keeping the single-qubit encoding fixed. More specifically, all variants retain the same Hadamard and \(R_Z\) feature-encoding layers, and differ only in the ZZ entangler pattern. This is the natural quantity to vary because PennyLane's \texttt{qml.IQPEmbedding} already allows the entangling wire pairs to be controlled through its \texttt{pattern} argument \parencite{pennylane_iqp_embedding}. The hyperparameter interface is also kept aligned across variants, in particular with respect to the number of IQP repeats and the SVM regularisation parameter.

\subsection{IQP Kernel Variants}

The first variant is the original \texttt{IQPKernelClassifier}, which uses the full IQP-style entangler pattern in every repeat of the embedding.

The second is \texttt{IQPKernelClassifierSeparable}. This variant keeps the same single-qubit IQP encoding but removes all ZZ couplings. It is therefore the direct no-entanglement ablation of the IQP embedding.

The third is \texttt{IQPKernelClassifierHalfSeparable}. This variant again keeps the same single-qubit IQP encoding, but in each repeat retains only a seeded random 50\% subset of the full ZZ edge set. As in the CCC case, the intention is to test whether performance degrades gradually as entangler density is reduced, or whether much of the kernel's behavior can already be recovered with a sparse entangling pattern.

These IQP-kernel variants are methodologically useful because they allow the entanglement question to be posed in a different QML model class. In CCC, entanglement appears inside a trainable variational ansatz. In the IQP-kernel model, the entangling ZZ structure is part of the fixed feature map itself. Comparing both families therefore helps separate claims about entanglement in a particular ansatz from claims about entanglement in a broader QML pipeline.

Listing~\ref{lst:iqp-embedding-ours-relevant} shows the base IQP-kernel implementation. Its single-qubit encoding and ZZ-entangler loop follow the relevant logic of PennyLane's \texttt{qml.IQPEmbedding} template~\parencite{pennylane_iqp_embedding}. Listing~\ref{lst:iqp-embedding-variants-relevant} then shows the variant-specific pattern overrides.

\begin{lstlisting}[
    style=pythonstyle,
    basicstyle=\ttfamily\scriptsize,
    caption={Relevant IQP-kernel logic implemented in \texttt{src/qml\_benchmarks/models/iqp\_kernel.py}.},
    label={lst:iqp-embedding-ours-relevant}
]
def _full_iqp_entangler_pattern(wires):
    # Full pairwise ZZ entangler pattern
...
class IQPKernelClassifier(ClassifierMixin, BaseEstimator):
    def _repeat_entangler_patterns(self, wires):
        # Use the same full pattern in every repeat
    ...
    def _apply_iqp_embedding(self, features, wires, repeat_patterns):
        for pattern in repeat_patterns:
            # As in qml.IQPEmbedding: Hadamards and RZ feature encoding
            for idx, wire in enumerate(wires):
                qml.Hadamard(wires=wire)
                qml.RZ(features[idx], wires=wire)

            # As in qml.IQPEmbedding: ZZ entanglers on the chosen pattern
            for wire_pair in pattern:
                idx_a = wires.index(wire_pair[0])
                idx_b = wires.index(wire_pair[1])
                angle = features[idx_a] * features[idx_b]
                qml.MultiRZ(angle, wires=wire_pair)
\end{lstlisting}
\begin{lstlisting}[
    style=pythonstyle,
    basicstyle=\ttfamily\scriptsize,
    caption={Variant-specific entangler-pattern logic for the separable and half-separable IQP-kernel classifiers.},
    label={lst:iqp-embedding-variants-relevant}
]
def _random_half_iqp_patterns(wires, n_repeats, random_state):
    ...
    # Keep about half of the full ZZ edge set in each repeat
    target_n_edges = max(1, len(full_pattern) // 2)
    patterns = []

    for repeat in range(n_repeats):
        rng = np.random.default_rng(random_state + 1000 * repeat)
        idx = rng.choice(len(full_pattern), target_n_edges, replace=False)
        idx = np.sort(idx)
...
class IQPKernelClassifierSeparable(IQPKernelClassifier):
    def _repeat_entangler_patterns(self, wires):
        # Remove all ZZ couplings in every repeat
        return tuple(tuple() for _ in range(self.repeats))


class IQPKernelClassifierHalfSeparable(IQPKernelClassifier):
    def _repeat_entangler_patterns(self, wires):
        # Keep a seeded random 50% subset of the full pattern
        return _random_half_iqp_patterns(
            wires, self.repeats, self.random_state
        )
\end{lstlisting}

Equivalence to the full IQP embedding is recovered when the repeat pattern in every block is the full pairwise wire pattern. In that case, the modified implementation applies the same Hadamard gates, the same single-qubit \(R_Z\) feature encodings, and the same \(ZZ\)-type \texttt{MultiRZ} gates as PennyLane's \texttt{qml.IQPEmbedding}, repeated the same number of times.
...
Model performance is evaluated primarily through test accuracy on the benchmark tasks. The central object of interest is the relative ranking of the original and dequantized CCC variants and IQP Kernel variants across datasets and benchmark settings. The experiments therefore examine not only absolute scores, but also whether removing entanglement changes the ranking of models across tasks of different difficulty.
...
Benchmark datasets were generated with a thesis-specific utility script built on top of the repository's data-generation functions. Its benchmark-specific parameter choices follow the original benchmark-generation setup from the paper benchmark scripts. Dataset generation used fixed seeds per benchmark family, including \texttt{42} for linearly separable and MNIST-PCA data, \texttt{3} for hidden-manifold and two-curves families, and \texttt{1} for hyperplanes diff.

Model-level randomness was controlled through the classifier parameter \texttt{random\_state}. In the controlled CCC ansatz, this parameter determines the seeded pseudorandom selection of active entangling edges, ensuring reproducibility of the selected edge subsets.

In the IQP-kernel family, \texttt{random\_state} initializes the model RNG, from which JAX PRNG keys and the SVM backend's random seed are derived. For the half-separable IQP variant, it also determines the seeded random 50\% subset of ZZ entangling edges selected in each IQP repeat through \path{_random_half_iqp_patterns}.

During final scoring, each selected model was evaluated over five runs with \texttt{random\_state} values \(0,\dots,4\). In addition, the hyperparameter-search and scoring scripts set \texttt{np.random.seed(42)} globally in order to make script-level NumPy randomness reproducible.
\chapter{Results}
...
\begin{figure}[htbp]
    \centering
    \parbox[t]{0.47\linewidth}{\centering \small \textbf{Linearly Separable}\par\includegraphics[width=0.96\linewidth]{../plot/figures/ranking-linearly-separable.png}}
    \parbox[t]{0.47\linewidth}{\centering \small \textbf{Hidden Manifold}\par\includegraphics[width=0.96\linewidth]{../plot/figures/ranking-hidden-manifold.png}}
    \parbox[t]{0.47\linewidth}{\centering \small \textbf{Hidden Manifold Diff}\par\includegraphics[width=0.96\linewidth]{../plot/figures/ranking-hmm-diff.png}}
    \parbox[t]{0.47\linewidth}{\centering \small \textbf{Hyperplanes Diff}\par\includegraphics[width=0.96\linewidth]{../plot/figures/ranking-hyperplanes-diff.png}}
    \parbox[t]{0.47\linewidth}{\centering \small \textbf{MNIST PCA}\par\includegraphics[width=0.96\linewidth]{../plot/figures/ranking-mnist-pca.png}}
    \parbox[t]{0.47\linewidth}{\centering \small \textbf{MNIST PCA Small}\par\includegraphics[width=0.96\linewidth]{../plot/figures/ranking-mnist-pca-small.png}}
    \parbox[t]{0.47\linewidth}{\centering \small \textbf{Two Curves}\par\includegraphics[width=0.96\linewidth]{../plot/figures/ranking-two-curves.png}}
    \parbox[t]{0.47\linewidth}{\centering \small \textbf{Two Curves Diff}\par\includegraphics[width=0.96\linewidth]{../plot/figures/ranking-two-curves-diff.png}}
...
\begin{figure}[htbp]
    \centering
    \parbox[t]{0.47\linewidth}{\centering \small \textbf{Linearly Separable}\par\includegraphics[width=0.96\linewidth]{../plot/figures/score-linearly_separable-ccc-qnn.png}}
    \parbox[t]{0.47\linewidth}{\centering \small \textbf{Hidden Manifold}\par\includegraphics[width=0.96\linewidth]{../plot/figures/score-hidden_manifold-qnn.png}}
    \parbox[t]{0.47\linewidth}{\centering \small \textbf{Hidden Manifold Diff}\par\includegraphics[width=0.96\linewidth]{../plot/figures/score-hidden_manifold_diff-qnn.png}}
    \parbox[t]{0.47\linewidth}{\centering \small \textbf{Hyperplanes Diff}\par\includegraphics[width=0.96\linewidth]{../plot/figures/score-hyperplanes_diff-qnn.png}}
    \parbox[t]{0.47\linewidth}{\centering \small \textbf{MNIST PCA}\par\includegraphics[width=0.96\linewidth]{../plot/figures/score-mnist_pca-qnn.png}}
    \parbox[t]{0.47\linewidth}{\centering \small \textbf{MNIST PCA Small}\par\includegraphics[width=0.96\linewidth]{../plot/figures/score-mnist_pca--qnn.png}}
    \parbox[t]{0.47\linewidth}{\centering \small \textbf{Two Curves}\par\includegraphics[width=0.96\linewidth]{../plot/figures/score-two_curves-qnn.png}}
    \parbox[t]{0.47\linewidth}{\centering \small \textbf{Two Curves Diff}\par\includegraphics[width=0.96\linewidth]{../plot/figures/score-two_curves_diff-qnn.png}}
...
\input{tables/train_test_gap_table}
...
For the IQP-kernel family, only the \(1d\), \(2d\), and \(3d\) linearly separable cases are completed for all three variants. On this small common subset, all models achieve high test accuracy, and the entanglement-reduced variants match or slightly exceed the original IQP model. Given the very limited number of completed IQP instances, Table~\ref{tab:iqp-linsep-small} should be read as a narrow summary rather than as a strong family-level conclusion.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{lccc}
        \hline
        Dimension & IQP & IQP Half-Separable & IQP Separable \\
        \hline
        \(1d\) & 0.9833 & 0.9833 & 0.9833 \\
        \(2d\) & 0.9667 & 0.9667 & 1.0000 \\
        \(3d\) & 0.9667 & 0.9800 & 0.9833 \\
        \hline
    \end{tabular}
    \caption{Mean test accuracy for the completed common subset of linearly separable IQP-kernel benchmark instances.}
    \label{tab:iqp-linsep-small}
\end{table}
...
The limited IQP results on the same family are broadly consistent with this reading. In the completed \(1d\)-\(3d\) IQP cases, the half-separable and fully separable variants match or slightly exceed the original IQP-kernel model. This suggests that, at least for these very simple low-dimensional linear-separation tasks, the full IQP entangling structure is not necessary to obtain strong performance. At the same time, the small number of completed IQP instances means that this observation should be treated cautiously and not generalized too far beyond the available cases.
...
This thesis investigated whether entanglement contributes to the performance of selected quantum machine-learning models. To this end, original CCC and IQP-kernel models were compared with partially and fully separable variants under a shared benchmarking pipeline. The aim was not to establish a general quantum advantage, but to examine whether entanglement influences model behavior when the remaining experimental conditions are kept fixed.

The main empirical analysis focused on the Circuit-Centric Classifier. Across the completed benchmark families, the original entangling CCC is usually the strongest model, the half-separable variant often remains competitive, and the fully separable model frequently suffers a substantial loss in performance. The results are therefore consistent with entanglement being relevant for the studied CCC models, especially on tasks where useful information must be combined across several input components. At the same time, the results do not suggest that the full entangling structure is always necessary. The half-separable CCC often remains close to the original model, indicating that sparse entangling connectivity can already preserve much of the effective capacity of the full architecture. The central distinction is therefore not simply between entangled and non-entangled models, but between different degrees of it. Removing entanglement entirely is often harmful, whereas reducing entangler density can still leave a competitive model. The effect of entanglement is also task-dependent. On simpler benchmark families such as linearly separable data, strong entangling connectivity appears less important, while on other families, such as hidden-manifold, MNIST PCA, and two-curves benchmarks, the fully separable model degrades more strongly. This suggests that entangling operations help the model combine information distributed across encoded quantum state, while the hyperplanes-diff benchmark suggests that some compositional boundary structures may still be captured by local rotations alone.

The IQP-kernel results have to be interpreted more cautiously. Only a small low-dimensional subset of the intended IQP experiments was completed. On these simple linearly separable cases, the entanglement-reduced IQP variants match or slightly exceed the original model, suggesting that the full IQP entangling pattern is not necessary for these particular tasks. However, the available IQP results are too limited to support a broader conclusion about quantum-kernel models.

Several directions follow naturally from this work. First, the IQP-kernel analysis should be extended to more benchmark families and higher dimensional settings. This would make it possible to test whether the patterns observed for the CCC family also appear in quantum-kernel models, where entanglement is part of the feature map rather than a trainable ansatz. Second, future work could study entanglement as a more fine-grained design parameter. Instead of comparing only fully entangling, half-separable, and fully separable variants, one could evaluate several entanglement levels between zero and one. This would show whether performance changes gradually with entangler density or whether threshold effects occur. Different connectivity patterns, such as nearest-neighbor, ring, random, or hardware-constrained layouts, could also be compared. Third, the readout structure of the separable CCC should be investigated more directly, particularly the first qubit measurement choice. Alternative readout rules, such as measuring several qubits or adding a classical post-processing layer, could clarify how much of the separable model's degradation comes from the absence of entanglement itself and how much from the restricted measurement rule. Nevertheless doing this while preserving the architecture as much as possible is another challenge. Future work could also complete benchmark families that were excluded due to computational cost, include more repeated runs to support stronger statistical conclusions, and test smaller instances under finite-shot sampling, noise models, or hardware-inspired connectivity constraints. A further extension would be to apply the same methodology to genuinely quantum data. This would test whether entanglement plays a different role when the data itself has quantum-mechanical structure.
