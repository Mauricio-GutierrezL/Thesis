...
Listing~\ref{lst:iqp-embedding-ours-relevant} gives the corresponding implementation in \texttt{src/qml\_benchmarks/models/iqp\_kernel.py}. It uses the relevant logic of PennyLane's \texttt{qml.IQPEmbedding} template~\parencite{pennylane_iqp_embedding} in \texttt{\_apply\_iqp\_embedding}. Listing~\ref{lst:iqp-embedding-variants-relevant} then shows the variant-specific pattern overrides.
...
Model-level randomness was controlled through the classifier parameter \texttt{random\_state}. In the controlled CCC ansatz, this parameter determines the seeded pseudorandom selection of active entangling edges, ensuring reproducibility of the selected edge subsets. In the IQP-kernel family, \texttt{random\_state} initializes the model RNG, from which JAX PRNG keys and the SVM backend's random seed are derived. For \texttt{IQPKernelClassifierHalfSeparable}, it also determines the seeded random 50\% subset of ZZ entangling edges selected in each IQP repeat through \texttt{\_random\_half\_iqp\_patterns}. During final scoring, each selected model was evaluated over five runs with \texttt{random\_state} values \(0,\dots,4\). In addition, the execution scripts \path{scripts/run_hyperparameter_search.py} and \path{scripts/score_with_best_hyperparameters.py} set \texttt{np.random.seed(42)} globally in order to make script-level NumPy randomness reproducible.
\chapter{Results}
