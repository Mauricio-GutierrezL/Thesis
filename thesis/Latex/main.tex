...
The \textbf{bars and stripes} benchmark moves from geometric toy data to simple image structure. Samples are noisy images belonging either to the class of vertical bars or to the class of horizontal stripes. As this benchmark requires exponentially more ressource to test, we prescind from it for our experiment.
...
This is the part of the tensor product construction. In the reference implementation used here, the original CCC employs PennyLane's \texttt{StronglyEntanglingLayers} as its variational block. The model output is computed from the expectation value of the Pauli-\(Z\)
...
the final score. 
For the purposes of this thesis, the important point is that the prediction rule is kept fixed across all CCC variants. The only systematic modification concerns the internal entangling structure of the variational circuit.
...
\begin{itemize}
  ...
  \item $\alpha = 1$ refers to all the entangling gates activated.
\end{itemize}
...
For CCC, the implementations are trained as binary classifiers with labels in $\{-1,+1\}$. The model parameters consist of the variational circuit parameters together with the scalar bias term. Hyperparameters such as the number of input copies, the number of variational layers, optimization settings, and related training parameters are selected through the same search procedure used by the benchmark framework. This point matters because, as emphasized by \textcite{bowles2024better}, hyperparameter tuning can strongly affect benchmark outcomes in QML.
...
\begin{figure}[H]
    ...
    \caption{Aggregate ranking of the three CCC variants across the completed benchmark set.}
    \label{fig:ccc-ranking-all}
\end{figure}

A more informative picture comes from the average accuracy differences. Relative to the original CCC, the half-separable random-50 variant shows only a small mean loss in test accuracy across the completed benchmark set, about \(-0.0127\). By contrast, the fully separable model is markedly worse, with a mean test-accuracy difference of about \(-0.1638\) relative to the original CCC. The difference between the half-separable and fully separable variants is correspondingly large, about \(+0.1510\) in favor of the half-separable model. The main empirical pattern is therefore not that every reduction of entanglement is equally harmful, but that \emph{removing entanglement entirely is much more damaging than thinning it out}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.49\linewidth]{../plot/figures/ranking-linearly-separable.png}
    \includegraphics[width=0.49\linewidth]{../plot/figures/ranking-hidden-manifold.png}
    \includegraphics[width=0.49\linewidth]{../plot/figures/ranking-hmm-diff.png}
    \includegraphics[width=0.49\linewidth]{../plot/figures/ranking-hyperplanes-diff.png}
    \caption{Per-family ranking plots for linearly separable, hidden manifold, hidden manifold diff, and hyperplanes diff.}
    \label{fig:ccc-ranking-geom}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.49\linewidth]{../plot/figures/ranking-mnist-pca.png}
    \includegraphics[width=0.49\linewidth]{../plot/figures/ranking-mnist-pca-small.png}
    \includegraphics[width=0.49\linewidth]{../plot/figures/ranking-two-curves.png}
    \includegraphics[width=0.49\linewidth]{../plot/figures/ranking-two-curves-diff.png}
    \caption{Per-family ranking plots for MNIST PCA, MNIST PCA small, two curves, and two curves diff.}
    \label{fig:ccc-ranking-vision-curves}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.49\linewidth]{../plot/figures/score-linearly_separable-ccc-qnn.png}
    \includegraphics[width=0.49\linewidth]{../plot/figures/score-hidden_manifold-qnn.png}
    \includegraphics[width=0.49\linewidth]{../plot/figures/score-hidden_manifold_diff-qnn.png}
    \includegraphics[width=0.49\linewidth]{../plot/figures/score-hyperplanes_diff-qnn.png}
    \caption{Train and test accuracy curves for the completed geometric benchmark families: linearly separable, hidden manifold, hidden manifold diff and hyperplanes diff.}
    \label{fig:ccc-scores-geom}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.49\linewidth]{../plot/figures/score-mnist_pca-qnn.png}
    \includegraphics[width=0.49\linewidth]{../plot/figures/score-mnist_pca--qnn.png}
    \includegraphics[width=0.49\linewidth]{../plot/figures/score-two_curves-qnn.png}
    \includegraphics[width=0.49\linewidth]{../plot/figures/score-two_curves_diff-qnn.png}
    \includegraphics[width=0.49\linewidth]{../plot/figures/score-bars_and_stripes-qnn.png}
    \caption{Train and test accuracy curves for MNIST PCA, MNIST PCA small, two curves, two curves diff, and bars and stripes.}
    \label{fig:ccc-scores-other}
\end{figure}
...
The main quantitative conclusion from the completed CCC data is therefore the following: the original entangling CCC is usually best, but the half-separable random-50 variant remains surprisingly competitive, whereas the fully separable model often incurs a substantial loss. Entanglement appears to matter, but the completed results suggest that \emph{full entangler density is often not required to obtain most of the performance of the original model}.
